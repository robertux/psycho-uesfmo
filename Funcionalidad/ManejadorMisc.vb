...
#region "CentroEstudios"
    ...
    Public Shared Function GetCentroEstudiosciudad(ByVal codciudad As String, ByVal tabla As DataTable) As List(Of centroestudio)
        Dim ce As New List(Of centroestudio)
        For Each fila As DataRow In tabla.Rows
            If (fila("codciudad") = codciudad) Then
                ce.Add(New centroestudio(fila("codigo"), fila("nombre")))
            End If
        Next
        Return ce
...
#Region "Departamentos/Ciudades"
    ...
    Public Shared Function GetCiudades(ByVal tabla As DataTable) As List(Of Ciudad)
        Dim ciudades As New List(Of Ciudad)
        For Each fila As DataRow In tabla.Rows
            ciudades.Add(New Ciudad(fila("codigo"), fila("nombre"), fila("coddepto")))
        Next
    ...
    Public Shared Function GetCiudadEstudios(ByVal codCentroEstudio As String, ByVal tablacentroestudios As DataTable, ByVal tablaciudad As DataTable) As Ciudad
        For Each fila As DataRow In tablacentroestudios.Rows
            If fila("codigo") = codCentroEstudio Then
                For Each fila2 As DataRow In tablaciudad.Rows
                    If fila2("codigo") = fila("codciudad") Then
                        Return New Ciudad(fila2("codigo"), fila2("nombre"), fila2("coddepto"))
                    End If
                Next
            End If
        Next
    ...
    Public Shared Function GetDeptoCiudad(ByVal codciudad As String, ByVal tablaciudad As DataTable, ByVal tabladepto As DataTable) As Departamento
        For Each fila As DataRow In tablaciudad.Rows
            If fila("codigo") = codciudad Then
                For Each fila2 As DataRow In tabladepto.Rows
                    If fila2("codigo") = fila("coddepto") Then
                        Return New Departamento(fila2("codigo"), fila2("nombre"))
                    End If
                Next
            End If
        Next
